...
public class AssetController : MonoBehaviour
{
    ...
    public List<Sprite> ListWeaponSprites = new List<Sprite>();
    ...
    private void Awake()
    {
        ...
        for (int i = 0; i < ListHeadsetItemData.Count; i++)
        {
            ...
            WeaponItems.Add(ListHeadsetItemData[i].ID, ListHeadsetItemData[i]);
        }
    ...
    {
        if (current < DataController.Instance.Data.HeadsetUnlockProgress[id] || !WeaponItems.ContainsKey(id)) return;
    ...
    public void AddWeapon()
    {
        ListHeadsetItemData.Add(new ShopItemData()
        {
            ...
            AvatarSprite = ListWeaponSprites.Count == 0 ? null : ListWeaponSprites[ListWeaponSprites.Count - 1],
            UCTotal = 1,
            Type = ItemType.Weapon
        });
        ListWeaponSprites.Add(ListWeaponSprites.Count == 0 ? null : ListWeaponSprites[ListWeaponSprites.Count - 1]);
    }

    public void RemoveWeapon(int index)
    {
        ListHeadsetItemData.RemoveAt(index);
        ListWeaponSprites.RemoveAt(index);
    }
}
